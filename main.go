...
func SelectByID(id int) bool {
	...
	result, err := db.Exec("select * from Items where id=$1", id)
	if err != nil {
		panic(err)
	}
...
func UpdateByID(item Item, id) {
	...
	result, err := db.Exec("update Items set Title = 'Carpet' where id=$2", item.Title, id)
	if err != nil {
		panic(err)
	}
	if val, _ := result.RowsAffected(); val < 1 {
